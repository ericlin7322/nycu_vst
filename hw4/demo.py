...
def main(args):
    ...
    width = 320
    height = 240
    output_m3u8 = "video/output.m3u8"
    
    process1 = (
        ffmpeg
        .input("/dev/video0", s="320x240")
        .output('pipe:', format='rawvideo', pix_fmt='rgb24', r=7)
        .run_async(pipe_stdout=True)
    )

    process2 = (
        ffmpeg
        .input('pipe:', format='rawvideo', pix_fmt='rgb24', s='{}x{}'.format(width, height), framerate=7)
        .output(output_m3u8, f='hls', g=0, hls_time=1, hls_init_time=0, hls_list_size=2, hls_flags='omit_endlist+delete_segments', hls_segment_filename='video/segment_%03d.ts')
        .overwrite_output()
    ...
    with open('tracking_ids.txt', 'w') as file1:
        pass
            ...
            for ret in res_track:
                ...
                for l in set1:
                    if tracking_id == l:
                        off = True
                        break;
                file.write(f'{tracking_id}, {int(bbox[0])}, {int(bbox[1])}, {int(bbox[2])}, {int(bbox[3])}, {width}, {height}\n')
